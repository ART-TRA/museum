...
const CURVE_PATH = [
  ...
  new THREE.Vector3(13.88, 1.4, 5.5), // 8+
  new THREE.Vector3(22.71, 1.4, 4.62), // 9+
...
const LOOK_AT = [
  ...
  new THREE.Vector3(10, 1.4, 13.5), // 8+
  new THREE.Vector3(25, 1.4, -2), // 9+
];

const ALLOW_INDEX = [1, 3, 5, 7, 8, 9];
...
export const TestPath = () => {
  ...
  useFrame((state, delta) => {
    if (allow) {
      ...
      const t = (timer.current % DURATION) / DURATION;
      const t2 = ((timer.current + 0.002) % DURATION) / DURATION;
      ...
      const targetLookAt = new THREE.Vector3()
        .subVectors(pos2, pos)
        .normalize();
      const lookAt = currentLookAt.lerp(targetLookAt, delta * friction);
  ...
  return (
    <group>
      ...
      {CURVE_PATH.map((coord, index) => (
        <mesh position={coord} key={index} visible={false}>
          <sphereGeometry args={[FRICTION_DISTANCE, 16, 16]} />
